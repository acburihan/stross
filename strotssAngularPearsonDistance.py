...
def pairwise_distances_angular_pearson(x, y):
    x_mean = torch.mean(x, dim=1, keepdim=True)
    y_mean = torch.mean(y, dim=1, keepdim=True)
    x_centered = x - x_mean
    y_centered = y - y_mean
    x_centered_sin = torch.sin(x_centered)
    y_centered_sin = torch.sin(y_centered)
    numerator = torch.mm(x_centered_sin, y_centered_sin.T)
    x_norm = torch.sqrt(torch.sum(x_centered_sin**2, dim=1, keepdim=True))
    y_norm = torch.sqrt(torch.sum(y_centered_sin**2, dim=1, keepdim=True))
...
def distmat(x, y, cos_d=True):
    if cos_d:
        M = pairwise_distances_angular_pearson(x, y)
    else:
...
def content_loss(feat_result, feat_content):
    ...
    Mx = distmat(X, X, cos_d=True)
    Mx = Mx#/Mx.sum(0, keepdim=True)

    My = distmat(Y, Y, cos_d=True)
