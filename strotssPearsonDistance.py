...
def pairwise_distances_pearson(x, y):
    x_mean = torch.mean(x, dim=1, keepdim=True)
    y_mean = torch.mean(y, dim=1, keepdim=True)
    x_centered = x - x_mean
    y_centered = y - y_mean
    numerator = torch.mm(x_centered, y_centered.T)
    x_norm = torch.sqrt(torch.sum(x_centered**2, dim=1, keepdim=True))
    y_norm = torch.sqrt(torch.sum(y_centered**2, dim=1, keepdim=True))
...
def distmat(x, y, cos_d=True):
    if cos_d:
        M = pairwise_distances_pearson(x, y)
    else:
